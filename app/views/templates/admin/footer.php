<script>
  ...
  $(document).ready(function() {
    ...
    $('.toggle_bars').on("click", function() {
      ...
      if(!$('#laporan_nav').hasClass('hidden') && !$('.sidebar').hasClass('sidebar_toggled')){
        if($('#laporan_nav').hasClass('hidden')){
          $('#logout_nav').removeClass('mt-16');
        }
      }
      if(!$('.sidebar').hasClass('sidebar_toggled') && !$('.submenu_laporan').hasClass('hidden')){
        $('#logout_nav').addClass('mt-16');
      }
      if($('.sidebar').hasClass('sidebar_toggled') && $('#logout_nav').hasClass('mt-16')){
        $('#logout_nav').removeClass('mt-16');
      }
      // if($('#laporan_nav').hasClass('hidden') && !$('.sidebar').hasClass('sidebar_toggled')){
      //   $('#logout_nav').removeClass('mt-16');
