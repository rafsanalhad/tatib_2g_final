      ...
      <div id="modalProses" class="fixed inset-0 hidden overflow-y-auto overflow-x-hidden z-50">
        ...
        <div class="flex items-center justify-center min-h-screen p-4">
          <div class="bg-white w-full max-w-3xl p-8 rounded-lg shadow-md relative">
            ...
            <form action="" id="formTolakPengaduan">
              <div class="tolakPengaduanContainer hidden mb-3">
                <input type="hidden" name="id" id="iniID" value="<?= $dt['pengaduan_id'] ?>">
                <input type="hidden" name="aksi" value="ditolak">
                <input type="hidden" name="idRiwayat" value="<?= $dt['riwayat_id']; ?>">
                <input type="text" name="catatan" id="catatanTolak" placeholder="Catatan" class="w-full h-10 border border-gray-400 rounded-lg mt-3 pl-2">
              </div>
              <div class="inline-flex justify-between w-full">
                <div class="text-left">
                  <button id="buttonTolakPengaduan" type="button" class="bg-orange-400 hover:bg-orange-700 text-white font-bold py-2 px-4 rounded">
                    Tolak Pengaduan
                  </button>
                </div>
    ...
    <script>
      ...
      // }
      $('#buttonTolakPengaduan').on("click", function() {
        $('.tolakPengaduanContainer').removeClass('hidden');
        $('#buttonTolakPengaduan').attr('type', 'submit');
      })
      $('#formTolakPengaduan').on('submit', function(e) {
        e.preventDefault();
        if ($('#catatanTolak').val() == '') {
        } else {
          $.ajax({
            url: '<?= BASEURL; ?>/Mahasiswa/tolakPengaduan',
            type: 'POST',
            data: $(this).serialize(),
              ...
              if (response == 'sukses') {
                Swal.fire({
                  ...
                  if (result.isConfirmed) {
                    window.location.href = '<?= BASEURL; ?>/mahasiswa/riwayatPelanggaran';
                  }
                });
              } else {
                alert(response);
